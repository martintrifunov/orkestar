...
func (m Model) forwardMouse(kind string, mouse tea.Mouse) bool {
	if m.embedded == nil || m.embedded.view == nil || !m.embedded.view.mouseEnabled() || m.prompting() || m.viewingHistory || m.viewingDiff || m.viewingExplanation || m.pickingAgent || m.pickingTemplate {
		return false
	}
	for _, r := range m.paneRects() {
