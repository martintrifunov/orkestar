...
// fixtureExecutable stands in for grok, which is not installed here
...
func TestCapabilitiesClaimOnlyWhatIsKnown(t *testing.T) {
	capabilities := grok.New("").Capabilities()
	if capabilities.Name != "grok" {
...
func TestLaunchesInteractively(t *testing.T) {
	...
	session, err := grok.New(fixtureExecutable(t)).Launch(ctx, agent.LaunchOptions{
...
func TestResumeIsRefusedRatherThanIgnored(t *testing.T) {
	...
	if _, err := grok.New(fixtureExecutable(t)).Launch(ctx, agent.LaunchOptions{
